...
if(isset($_SESSION['id']) && in_array(55, $permission))
{
...
include_once("export_modal.php");
    ...
    $export_query = mysqli_query($conn,"DESCRIBE tbl_facility_location");
    while($fetch_export = mysqli_fetch_array($export_query)) {
        $col_name = $fetch_export['Field'];
        $tbl_name = 'tbl_facility_location';
...
?>
    ...
    <div class="container-fluid">
        ...
        <div class="d-flex align-items-center justify-content-between mb-3">
            <h4 class="mb-0 text-gray-800">Facility List</h4>
            <button type="button" class="d-sm-inline-block btn btn-sm btn-warning shadow-sm text-dark add-btn" data-toggle="modal" <?php if(in_array(53, $permission)){ echo 'data-target="#addModal"'; }else{ echo 'data-target="#alertModal"'; } ?>><i class="fa fa-plus"></i> Add Facility</button>
        </div>
        <hr>
        ...
        if(!empty($_GET['status'])){
            switch($_GET['status']){
                ...
                case 'update':
                    $statusType = 'alert-success';
                    $statusMsg = '<i class="fa fa-check-circle fa-sm"></i>&nbsp;<b>Success!</b> Facility has been updated successfully.';
                    break;
                case 'description':
                    $statusType = 'alert-success';
                    $statusMsg = '<i class="fa fa-check-circle fa-sm"></i>&nbsp;<b>Success!</b> Description has been added successfully.';
                    break;
                case 'err':
        ...
        <?php } ?>
                ...
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <div class="modal-header bg-warning">
                                <h6 class="modal-title text-dark" id="exampleModalLabel"><i class="fas fa-plus"></i> &nbsp;Add Facility Location</h6>
                        ...
                        <form method="POST" action="facility_add.php" id="WarehouseLockForm">
                            ...
                            <div class="modal-footer">
                                ...
                                <button class="btn btn-success btn-sm" name="submit" type="submit">Add Facility</button>
                            </div>
                        </form>
                    </div>
                </div>
            ...
            <?php
            $update_query = mysqli_query($conn,"SELECT * FROM tbl_facility_location");
                ...
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <div class="modal-header bg-dark">
                            <h6 class="modal-title text-warning" id="exampleModalLabel"><i class="fa fa-cog fa-sm"></i> Update Facility</h6>
                            <button class="close" type="button" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true"><small>×</small></span>
                            </button>
                        </div>
                        <form method="POST" action="facility_update.php">
                            <div class="modal-body">
            ...
            ?>
            <!-- Description Modal -->
            <div class="modal fade" id="description<?php echo $fetch_desc['id'];?>" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
                aria-hidden="true">
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <div class="modal-header bg-warning">
                        ...
                        <form method="POST" action="facility_description.php">
                            <div class="modal-body">
                                ...
                                <div class="form-group">
                                    <div class="form-row">
                                        <div class="col-12">
                                            ...
                                            <input type="text" class="form-control form-control-sm" name="description" autocomplete="off" required>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="modal-footer">
                                <input type="text" value="<?php echo $fetch_desc['id'];?>" name="facility_id" hidden>
                                <button class="btn btn-secondary btn-sm" type="button" data-dismiss="modal">Cancel</button>
                                <button class="btn btn-success btn-sm" name="submit" type="submit">Add Description</button>
                            </div>
                        </form>
                    </div>
                </div>
            ...
            ?>
                ...
                <div class="card shadow mb-4">
                    <div class="card-header py-2 bg-primary">
                        <h6 class="m-0 font-weight-bold text-light">Existing Facility List</h6> 
                    </div>
                    <div class="card-body">
                        <div class="d-flex flex-row-reverse">
                            <a type="button" class="btn btn-sm btn-light" data-toggle="modal" <?php if(in_array(56, $permission)){ echo 'data-target="#exportModal"'; }else{ echo 'data-target="#alertModal"'; } ?>><i class="fa fa-download"></i> Export Data</a>&nbsp;|&nbsp; 
                            <a type="button" class="btn btn-sm btn-light" <?php if(in_array(59, $permission)){ echo 'href="facility_raw.php"'; }else{ echo 'data-toggle="modal" data-target="#alertModal"'; } ?>><i class="fa fa-file"></i> Report Data</a>
                        </div>
                        <hr>
                        <div class="table-responsive">
                            <table class="table table-striped table-bordered table-sm text-center" id="dataTable" width="100%" cellspacing="0">
                                ...
                                <tbody>
                                    <?php
                                        $result = mysqli_query($conn,"SELECT * FROM tbl_facility_location WHERE location IN (" . $location . ")");
                                        while($row = mysqli_fetch_array($result)){
                                            ...
                                            ?>
                                            <td class="align-middle"><center><button type="button" class="d-sm-inline-block btn btn-sm btn-info" data-toggle="modal" <?php if(in_array(53, $permission)){ echo "data-target=#description".$row['id']; }else{ echo 'data-target="#alertModal"'; } ?>><i class="fa fa-list fa-sm fa-fw"></i>View List</button></td></center>

                                            <td class="align-middle"><center><button type="button" class="d-sm-inline-block btn btn-sm btn-outline-warning" data-toggle="modal" <?php if(in_array(54, $permission)){ echo "data-target=#update".$row['id']; }else{ echo 'data-target="#alertModal"'; } ?>><i class="fa fa-cog fa-sm fa-fw"></i> Update</button></td></center>
