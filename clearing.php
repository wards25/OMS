...
?>
    ...
    <div class="container-fluid">
        ...
        <div class="d-flex align-items-center justify-content-between mb-3">
            <h4 class="mb-0 text-gray-800">To Clear</h4>
                    ...
                    <div class="card-body">
                        <div class="table-responsive">
                            <table class="table table-striped table-bordered table-sm text-center" id="dataTable" width="100%" cellspacing="0">
                                ...
                                <tbody>
                                    <?php
                                        $result = mysqli_query($conn, "SELECT * FROM tbl_trips_tm WHERE clearing_status = '' ORDER BY dtr DESC");
                                        while ($row = mysqli_fetch_assoc($result)) {
                                            echo '<tr>';
                                            echo '<td><center>' . $row['dispatch_date'] . '</td>';
                                            echo '<td><center>' . $row['tmno'] . '</td>';
                                            echo '<td><center>' . $row['trucktype'] . '</td>';
                                            echo '<td><center>' . $row['plateno'] . '</td>';
                                            echo '<td><center>' . $row['driver'] . '</td>';

                                            if ($row['clearing_status'] == 'TO CLEAR' || empty($row['clearing_status'])) {
                                                echo '<td><center><span class="badge badge-warning">TO CLEAR</span></center></td>';
                                            } else {
                                                echo '<td><center><span class="badge badge-success">' . $row['clearing_status'] . '</span></center></td>';
                                            }

                                            echo '<td><center>
                                                    <a type="button" name="view" class="btn btn-sm btn-info" 
                                                       onclick="window.open(\'clearing_view.php?tmno=' . $row['tmno'] . '&dtr=' . $row['dtr']. '\', \'_blank\')">
