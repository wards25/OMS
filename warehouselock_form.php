...
if(isset($_SESSION['id']) && $check_submit == 0 && in_array(25, $permission))
{
...
?>
    ...
    <div class="container-fluid">
        ...
        <div class="d-flex align-items-center justify-content-between mb-3">
            ...
            <a type="button" href="checklist_admin.php" class="d-sm-inline-block btn btn-sm btn-secondary shadow-sm add-user-btn"><i class="fa fa-arrow-left"></i> Back</a>
        </div>
        <hr>
        ...
        if(!empty($_GET['status'])){
            switch($_GET['status']){
                case 'add':
                    ...
                    $statusMsg = '<i class="fa fa-check-circle fa-sm"></i>&nbsp;<b>Success!</b> Warehouse Lock has been added successfully.';
                    break;
                case 'irsucc':
                    $statusType = 'alert-success';
                ...
                case 'err':
                    ...
                    $statusMsg = '<i class="fa fa-exclamation-triangle fa-sm"></i>&nbsp;<b>Error!</b> Warehouse Lock exists.';
                    break;
                default:
                    $statusType = '';
                    $statusMsg = '';
        ...
        <?php } ?>
                ...
                <div class="card shadow mb-4">
                    ...
                    <div class="card-body">
                        ...
                        <form method="POST" action="warehouselock_submit.php" enctype="multipart/form-data">
                        <div class="table-responsive">
                            <table class="table table-striped table-bordered table-sm text-center" width="100%" cellspacing="0">
                                ...
                                <tbody>
                                    <?php
                                        $result = mysqli_query($conn,"SELECT * FROM tbl_warehouselock_location WHERE location = '$location' AND is_active = '1'");
                        ...
                        <hr>
                        <center>
                            <button type="button" class="d-sm-inline-block btn btn-sm btn-warning text-dark add-btn" data-toggle="modal" <?php if(in_array(11, $permission)){ echo 'data-target="#addWhseLock"'; }else{ echo 'data-target="#alertModal"'; } ?>><i class="fa fa-plus"></i> Add Location</button>
                            <?php
                            ...
                            if ($fetch_deadline = mysqli_fetch_array($deadline_query)) {
                                ...
                                if ($time_now >= $time_from && $time_now <= $time_to) {
                                    echo '<a class="d-sm-inline-block btn btn-info btn-sm" data-toggle="modal" data-target="#OperationModal"><i class="fa-solid fa-clock"></i> 24 hrs Operation</a>&nbsp;';
                                    ...
                                    echo '<a class="d-sm-inline-block btn btn-info btn-sm" data-toggle="modal" data-target="#DeadlineModal"><i class="fa-solid fa-clock"></i> 24 hrs Operation</a>&nbsp;';
                ...
                <!-- Operation Modal -->
                <div class="modal fade" id="OperationModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
                    aria-hidden="true">
                    <div class="modal-dialog" role="document">
                        <div class="modal-content">
                            <div class="modal-header bg-info">
                                <h6 class="modal-title text-light" id="exampleModalLabel"><i class="fas fa-truck fa-sm"></i> 24 hrs Operation</h6>
                                <button class="close" type="button" data-dismiss="modal" aria-label="Close">
                                    <span aria-hidden="true"><small>×</small></span>
                                </button>
                            </div>
                            <form method="POST" action="warehouselock_submit.php">
                            <div class="modal-body">Do you want to bypass this shift?</div>
                            <div class="modal-footer">
                                <input type="text" name="location" value="<?php echo $location; ?>" hidden>
                                <input type="text" name="date" value="<?php echo $date; ?>" hidden>
                                <input type="text" name="shift" value="<?php echo $shift; ?>" hidden>
                                <input type="text" name="shift_type" value="<?php echo $shift_type; ?>" hidden>
                                <button class="btn btn-secondary btn-sm" type="button" data-dismiss="modal">Cancel</button>
                                <button class="btn btn-success btn-sm" type="submit" name="bypass">Bypass</button>
                            </div>
                            </form>
                        </div>
                    </div>
                    ...
                    <div class="modal-dialog" role="document">
                        <div class="modal-content">
                            <div class="modal-header bg-warning">
                                <h6 class="modal-title text-dark" id="exampleModalLabel"><i class="fa-solid fa-shop-lock fa-sm"></i> &nbsp;Add Warehouse Lock</h6>
                                <button class="close" type="button" data-dismiss="modal" aria-label="Close">
                                    <span aria-hidden="true"><small>×</small></span>
                                </button>
                            </div>
