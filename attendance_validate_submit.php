...
if(isset($_POST['submit'])){
    ...
    foreach ($employee_values as $id => $name)
    {
        ...
        $exist_query = mysqli_query($conn,"SELECT * FROM tbl_attendance_raw WHERE date='$date' AND location='$location' AND shift='$shift' AND shift_type='$shift_type' AND employee_name='$employee_name'");
        $fetch_id = mysqli_fetch_array($exist_query);

        mysqli_query($conn,"UPDATE tbl_attendance_raw SET attendance='$attendance',reason='$remarks',uniform='$uniform',identification='$identification',vest='$vest',safety_shoes='$safety_shoes',remarks='$edit',is_validated='1',validated_by='$user',validated_at=NOW() WHERE id=".$fetch_id['id']);

    }

    mysqli_query($conn,"UPDATE tbl_attendance SET is_validated='1',validated_by='$user',validated_at=NOW() WHERE date='$date' AND location='$location' AND shift='$shift' AND shift_type='$shift_type'");
