...
if(isset($_POST['submit'])){
    ...
    $check_query = mysqli_query($conn,"SELECT * FROM tbl_ewt_raw WHERE order_no = '$order_no' AND status <= '0'");
    ...
    if ($row >= 1){
        ...
        // Directory
        // $directory = 'C:/xampp/htdocs/oms/upload/ewt/';
        $directory = 'D:/public/www/oms.ramosco.net/upload/ewt/';
        ...
        if (isset($_FILES['ewt']) && is_array($_FILES['ewt']['name'])) {
            ...
            for ($i = 0; $i < $total_files; $i++) {
                if ($_FILES['ewt']['error'][$i] === 0 && !empty($_FILES['ewt']['name'][$i])) {
                    ...
                    // New filename format
                    $new_filename = "{$order_no}_{$order_date}_" . ($i + 1) . "." . $ext;
                    $destination = $filelocation . $new_filename;

                    // Move file to final destination
                    if (move_uploaded_file($tmp_name, $destination)) {
                        // ✅ Use relative path for database (so it works on the web)
                        $uploaded_file_paths[$i] = "upload/ewt/$website/$order_no/$new_filename";
                    }
                }
            }
        }
        ...
        // Insert record
        $query = "
        INSERT INTO tbl_ewt_raw 
        (id, website, serial_no, uploader, uploader_email, order_no, order_date, 
         ewt1, ewt2, ewt3, ewt4, ewt5, upload_date, upload_time, 
         validated_by, validated_date, invoiced_by, invoiced_date, status, remarks)
        VALUES 
        (NULL, '$website', '$serial_no', '$uploader_name', '$uploader_email',
         '$order_no', '$order_date', 
         '$ewt1', '$ewt2', '$ewt3', '$ewt4', '$ewt5',
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['email'])) {
            ...
            if (filter_var($recipient, FILTER_VALIDATE_EMAIL)) {
                ...
                try {
                    ...
                    $mail->CharSet = 'UTF-8';
                    $mail->isHTML(true);
                    $mail->setFrom('no-reply@ramoscogroup.com', 'EWT Submission');
                        ...
                        $mail->Body = '
                            <div style="font-family:Helvetica Neue, Helvetica, Arial, sans-serif; padding:20px; background-color:#edfaf4;">
                                <div style="max-width:600px; margin:auto; padding:30px; border-radius:8px; background-color:#ffffff">
                                    ...
                                    <br>
                                    <small>Thank you for submitting your EWT!</small>
                                    <hr>

                                    <p style="font-size:13px;">Dear '.$uploader_name.',</p>

                                    <p style="font-size:13px;">Your EWT for the transcation <strong>'.$order_no.'</strong> has been submitted successfully!</p>
        ...
        header("Location: ewt_success.php");
    }
}else{
    $qstring = '?status=tag';
