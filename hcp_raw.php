...
?>
    ...
    <div class="container-fluid">
        ...
        <div class="d-flex align-items-center justify-content-between mb-3">
            <h4 class="mb-0 text-gray-800">HCP Individual</h4>
                        ...
                        <div class="d-flex flex-wrap justify-content-between align-items-start" style="gap: 10px;">
                            <div class="flex-grow-1 min-w-0 me-md-3">
                                <div class="table-responsive">
                                    ...
                                    $validated_count = 0;
                                    $enrolled_count = 0;
                                    $data_query = mysqli_query($conn,"SELECT validated FROM tbl_customer WHERE tag = 'HCP INDIVIDUAL'");
                                    while($fetch_data = mysqli_fetch_assoc($data_query)){
                                        if($fetch_data['validated'] == 0){
                                            $notvalidated_count += 1;
                                        }else if($fetch_data['validated'] == 1){
                                            $validated_count += 1;
                                        }else{
                                            $enrolled_count += 1;
                                        }
                                    }
                                    $total_count = $notvalidated_count + $validated_count + $enrolled_count;
                                    ?>
                                    <table class="table table-bordered table-sm mb-0" style="min-width: 100%; max-width: 100%;">
                                        <tbody>
                                            <tr>
                                                <td class="table-warning"><b>Enrolled:</b></td>
                                                <td class="text-center"><b><?php echo $enrolled_count; ?></b></td>
                                            ...
                                            <tr>
                                                <td class="table-info" colspan="3"><b>Total Registered:</b></td>
                            ...
                            <div class="flex-shrink-0">
                                <a type="button" class="btn btn-sm btn-light mt-2 mt-md-0" data-toggle="modal"
                                   <?php if(in_array(168, $permission)){ echo 'data-target="#exportModal"'; }else{ echo 'data-target="#alertModal"'; } ?>>
                            ...
                            <table class="table table-striped table-bordered table-sm text-center" id="dataTable" width="100%" cellspacing="0">
                                ...
                                <tbody>
                                    <?php
                                        ...
                                        $result = mysqli_query($conn,"SELECT * FROM tbl_customer WHERE tag='HCP INDIVIDUAL'");
                                        while($row = mysqli_fetch_assoc($result)) {
                                            echo '<tr>';
                                            echo '<td>'.$row['serial_no'].'</td>';
                                            echo '<td><center>'.$row['business_name'].'</center></td>';
                                            // echo '<td><center>'.$row['authorized_name'].'</center></td>';
                                            echo '<td><center>'.$row['authorized_contact'].'</center></td>';

                                            $validity_date = strtotime($row['validity_date']);
                                            $diffInSeconds = $validity_date - $currentDateTimestamp;
                                            $remainingDays = round($diffInSeconds / (60 * 60 * 24));

                                            if ($remainingDays > 0) {
                                                echo '<td><center><span class="badge badge-success">'.$remainingDays.' Days Rem</span></center></td>';
                                            } elseif ($remainingDays < 0) {
                                                echo '<td><center><span class="badge badge-danger">'.abs($remainingDays).' Days Exp</span></center></td>';
                                            } else {
                                                echo '<td><center><span class="badge badge-pending">Expiring Today</span></center></td>';
                                            }

                                            if($row['validated'] == 1){
                                                echo '<td><center><span class="badge badge-success">Validated</span></center></td>';
                                            }else if($row['validated'] == 2){
                                                echo '<td><center><span class="badge text-white" style="background-color:#cc589c;">Enrolled</span></center></td>';
                                            }else{
                                                echo '<td><center><span class="badge badge-danger">Not Validated</span></center></td>';
                                            }
                                            ?>
                                            <td><center><a class="d-sm-inline-block btn btn-sm btn-warning" name="edit" type="button" <?php if (in_array(166, $permission)) { echo 'href="cif_update.php?link=hcp_raw.php&tag='.$row['tag'].'&ref_no='.$row['serial_no'].'"'; } else {  echo 'data-toggle="modal" data-target="#alertModal"'; } ?>><i class="fa fa-edit fa-sm"></i></a></center></td>
                                            <td><center><a class="d-sm-inline-block btn btn-sm btn-info" name="view" type="button" <?php if (in_array(167, $permission)) { echo 'href="cif_view.php?link=hcp_raw.php&tag='.$row['tag'].'&ref_no='.$row['serial_no'].'"'; } else {  echo 'data-toggle="modal" data-target="#alertModal"'; } ?>><i class="fa fa-eye fa-sm"></i></a></center></td>
