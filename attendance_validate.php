...
if(isset($_SESSION['id']) && in_array(31, $permission))
{
...
?>
    ...
    <div class="container-fluid">
        ...
        <?php } ?>
                    ...
                    <div class="card-body">
                        <div class="table-responsive">
                            <table class="table table-bordered table-sm" width="100%" cellspacing="0">
                                <?php
                                $details_query = mysqli_query($conn,"SELECT * FROM tbl_attendance_raw WHERE date='$date' AND location='$location' AND shift='$shift' AND shift_type='$shift_type' GROUP BY submitted_by");
                        ...
                        <form method="POST" action="attendance_validate_submit.php" enctype="multipart/form-data">
                        <div class="table-responsive">
                            <table class="table table-striped table-bordered table-sm text-center" width="100%" cellspacing="0">
                                ...
                                <tbody>
                                    <?php
                                        $result = mysqli_query($conn, "SELECT * FROM tbl_attendance_raw WHERE date='$date' AND location='$location' AND shift='$shift' AND shift_type='$shift_type'");
                                        while ($row = mysqli_fetch_assoc($result)) {
                                            ...
                                            <td class="align-middle"><center><input type="checkbox" name="vest[<?php echo $row['id']; ?>]" value="1" <?php echo ($row['vest'] == 1) ? 'checked' : ''; ?> class="vest-checkbox"></center></td>
                                            <td class="align-middle"><center><input type="checkbox" name="safety_shoes[<?php echo $row['id']; ?>]" value="1" <?php echo ($row['safety_shoes'] == 1) ? 'checked' : ''; ?> class="safety-shoes-checkbox"></center></td>
                        ...
                        <hr>
                        <center>
                            <a class="d-sm-inline-block btn btn-info btn-sm" data-toggle="modal" data-target="#ImageModal"><i class="fa fa-sm fa-paperclip"></i> Image</a>
                            <?php
                ...
                $new_file_name = $location_name.'_'.$date.'-'.$shift.'_'.$shift_type_folder.'.jpg';
                //$filelocation = "C:/public/www/f325.ramosco.net/filepicture/dbapps/";
                $filelocation = 'upload/'.$location.''.$year.''.$month.''.$date_folder.''.$shift_folder.'/attendance/';
                    ...
                    <div class="modal-dialog" role="document">
                        <div class="modal-content">
                            <div class="modal-header bg-info">
                                <h6 class="modal-title text-light" id="exampleModalLabel"><i class="fa fa-sm fa-image"></i> Attachment</h6>
                                <button class="close" type="button" data-dismiss="modal" aria-label="Close">
                                    <span aria-hidden="true"><small>×</small></span>
                                </button>
                            </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            attendanceCheckboxes.forEach(function(attendanceCheckbox) {
                attendanceCheckbox.addEventListener('change', function() {
                    ...
                    document.querySelector(`input[name="vest[${id}]"]`).checked = isChecked;
                    document.querySelector(`input[name="safety_shoes[${id}]"]`).checked = isChecked;
                });
            });
        });
        ...
        $(document).ready(function() {
            ...
            $(document).on('change', '.uniform-checkbox, .identification-checkbox, .vest-checkbox, .safety-shoes-checkbox', function() {
