...
if(isset($_SESSION['id']) && in_array(43, $permission))
{
...
?>
    ...
    <div class="container-fluid">
        ...
        <div class="d-flex align-items-center justify-content-between mb-3">
            <h4 class="mb-0 text-gray-800">Temp Monitoring Report</h4>
                    ...
                    <div class="card-body">
                        <div class="table-responsive">
                            <table class="table table-bordered table-sm" width="100%" cellspacing="0">
                                <?php
                                $details_query = mysqli_query($conn,"SELECT * FROM tbl_temp_raw WHERE date='$date' AND location='$location' AND shift='$shift' AND shift_type='$shift_type' GROUP BY submitted_by");
                        ...
                        <div class="table-responsive">
                            <table class="table table-striped table-bordered table-sm text-center" width="100%" cellspacing="0">
                                ...
                                <tbody>
                                    <?php
                                    $result = mysqli_query($conn,"SELECT * FROM tbl_temp_raw WHERE date='$date' AND location='$location' AND shift='$shift' AND shift_type='$shift_type'");
                        ...
                        <div class="table-responsive">
                            <table class="table table-bordered table-sm" width="100%" cellspacing="0">
                                <tbody>
                                    <?php
                                    $temp_query = mysqli_query($conn,"SELECT * FROM tbl_temp_raw WHERE date='$date' AND location='$location' AND shift='$shift' AND shift_type='$shift_type' ORDER BY location_name");
                                    while($row = mysqli_fetch_array($temp_query)){
                                        $zone = $row['location_name'];
                                        $averagepos = $row['temp'];
                                        ?>
                                    <tr>
                                        <td class="text-danger font-weight-bold"><i><?php echo $zone.' Score'; ?></i></td>
                                    <?php
                                        $metric = 'Temp: '.$zone;
                                        $kpi_query = mysqli_query($conn,"SELECT * FROM tbl_kpi WHERE metric = '$metric'");
