...
?>
        ...
        <?php } ?>
                ...
                <div class="card shadow mb-4">
                    ...
                    <div class="card-body">
                        ...
                        <form method="POST" action="asset_validate_submit.php">
                        <div class="table-responsive">
                            <table class="table table-striped table-bordered table-sm text-center" width="100%" cellspacing="0">
                                ...
                                <tbody>
                                    <?php
                                    $result = mysqli_query($conn,"SELECT * FROM tbl_asset_inv WHERE asset_type='Warehouse' AND location = '$location'");
                                    while($row = mysqli_fetch_assoc($result)) {
                                        ...
                                        $validate_query = mysqli_query($conn,"SELECT * FROM tbl_asset_raw WHERE date='$date' AND location='$location' AND shift='$shift' AND shift_type='$shift_type' AND asset_id ='$subject'");
                                        $fetch_validate = mysqli_fetch_array($validate_query);
                                    ?>
                                        ...
                                        <select class="form-control form-control-sm" name="employee_name[<?php echo $row['id']; ?>]" id="employee_select_<?php echo $row['id']; ?>">
                                            <option value="<?php echo $fetch_validate['assigned_to']; ?>"><?php echo $fetch_validate['assigned_to']; ?> (default)</option>
                                            <option value=""></option>
                                            <?php    
                                            ...
                                            echo '</select>';
                                        ?>
                                            <td class="align-middle"><center><input type="checkbox" name="condition[<?php echo $row['id']; ?>]" value="1" checked></center></td>
                                            <?php
                                            $subject = $row['asset_id'];
    ...
    <script>
       document.addEventListener('DOMContentLoaded', function() {
            // Function to update dropdowns and hide selected options from other dropdowns
            function updateDropdowns() {
                // Get all select elements
                var selects = document.querySelectorAll('select[name^="employee_name"]');
                
                // Get all selected values, excluding "Not In Use"
                var selectedValues = [];
